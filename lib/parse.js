...
let getViewForSwagger2 = function (opts) {
  ...
  _.forEach(swagger.paths, function (api, path) {
    ...
    _.forEach(api, function (op, m) {
      ...
      // if (responses[200] && responses[200].hasOwnProperty('schema')) {
      //   if(responses[200].schema)
      //     method.response$ref = responses[200].schema.$ref
      //   }
      //处理response
      let responses = handleResponse(op.responses).match(/\#\/definitions\/(.*)\"/)
      method.res_body = responses ? responses[1].slice(0, -5).replace(responses[1].slice(0, -5)) : ''
      if (responses) {
        const temp = responses[1].slice(0, -5)
        method.res_body = temp.replace(temp[0], temp[0].toLowerCase())
      } else {
        method.res_body = ''
      }
